/*
 * $ Header: it.geosolutions.georepo.gui.client.widget.map.MapLayoutWidget,v. 0.1 25-gen-2011 11.30.33 created by afabiani <alessio.fabiani at geo-solutions.it> $
 * $ Revision: 0.1 $
    ...
    private void initDrawFeatures(final boolean isGoogle) {
        FeatureAddedListener listener = new FeatureAddedListener() {
            public void onFeatureAdded(VectorFeature vf) {
                ...
                Dispatcher.forwardEvent(GeoRepoEvents.INJECT_WKT, aoi.toString());

                Dispatcher.forwardEvent(GeoRepoEvents.DISABLE_DRAW_BUTTON);
            }
        };
