    ...
    private GridCellRenderer<Rule> createUsersCustomField() {
        GridCellRenderer<Rule> comboRendered = new GridCellRenderer<Rule>() {
            ...
            public Object render(final Rule model, String property, ColumnData config,
                    ...
                    grid.addListener(Events.ColumnResize, new Listener<GridEvent<Rule>>() {
                        ...
                        public void handleEvent(GridEvent<Rule> be) {
                            for (int i = 0; i < be.getGrid().getStore().getCount(); i++) {
                                ...
                                if (be.getGrid().getView().getWidget(i, be.getColIndex()) != null
                                        && be.getGrid().getView().getWidget(i, be.getColIndex()) instanceof CustomField) {
                                    ((CustomField) be.getGrid().getView().getWidget(i,
                                            be.getColIndex())).setWidth(be.getWidth() - 10);
                                }
                            }
                        }
                ...
                CustomField<Rule> usersCustomField = new CustomField<Rule>(list);// (ListField)
    ...
    private static final class CustomField<T> extends TextField<T> {
        ...
        private CustomField(List list) {// ListField users
            ...
            while (itr.hasNext()) {
                ...
                /*
                 * LabelField lf = new LabelField(); lf.setText(name);
                 * lf.setStyleAttribute("position", "absolute"); lf.addListener(Events.OnClick, new
                 * Listener<FieldEvent>(){ public void handleEvent(FieldEvent be) {
                 * System.out.println("lf di '"+be.getField().getValue()+"' clecked"); }
                 * 
                 * });
                 */
                String name = r.getUser() != null ? r.getUser().getName() != null ? r.getUser()
                        .getName() : "*" : "*";
             ...
             * + "<div>"+lf.getText()+"</div>"+ "<div>"+lf2.getText()+"</div>" +
             ...
             * + "</div>"); panel.add(lf); panel.add(lf2);
             */
            ...
            first.show();
            /*
             * lf.addListener(Events.OnClick, new Listener<FieldEvent>(){ public void
             * handleEvent(FieldEvent be) { System.out.println("lf clecked"); }
             * 
             * }); lf2.addListener(Events.OnClick, new Listener<FieldEvent>(){ public void
             * handleEvent(FieldEvent be) { System.out.println("lf clecked"); }
             * 
             * });
             */
             ...
             * 
             * 
             * public void handleEvent(FieldEvent be) { System.out.println("lfield OnClick");
             * lfield.show(); System.out.println("lfield OnClick end"); } });
             * h.addListener(Events.OnClick, new Listener<FieldEvent>(){
             * 
             * 
             * public void handleEvent(FieldEvent be) { System.out.println("h OnClick"); h.show();
             * System.out.println("h OnClick end"); } }); container.addListener(Events.OnClick, new
             * Listener<FieldEvent>(){
             * 
             * 
             * public void handleEvent(FieldEvent be) { System.out.println("panel OnClick");
             * container.show(); panel.show(); System.out.println("panel OnClick end"); } });
             */
